...
Workspace YAML may store the selected `sidebar.name`; absent selection uses the suggested `🤖 Bots` name during guided setup. Bot names and session roles already supply readable identity. Keep the observed native section ID, surface and placement evidence in a minimal separate YAML receipt, not as an invented portable identifier or another bot directory. Changing display organization never changes bot roots or stable native IDs.
...
Verify requested permissions against the actual task's exposed execution settings and representative command/tool behavior. Parent-task permissions, an input-box label and native MCP approval rules are distinct observations. Preserve the user's selection, record unsupported readback or a mismatch, and use a supported task-specific recovery step without changing global defaults or bypassing a pending approval. Repeated one-off approvals do not prove that the requested Full Access mode took effect.

After matching each managed bot project, use the actual host's native custom-section capabilities to create or reuse the selected group and move only the verified managed projects into it. Resolve prior section identity and actual readback before name matching; missing, renamed or ambiguous sections require reconciliation. Repeated setup and later bot creation reuse the same selected group. Record the execution host and the user-selected client views, including another desktop or mobile when used to access that local host. Verify project visibility, daily-conversation discoverability/openability and group placement separately in each selected view through relevant native evidence or explicit user observation. A successful move call, host-local inventory, exact-ID conversation read or pinning alone does not establish presentation on another client. Do not automatically reorganize other projects or infer authority to rename a shared group.

Current host-local inventory observations support a named section with emoji, moving projects into it and naming conversations. They do not establish remote presentation: the owner's later report records task access through Recent chats on another desktop, missing project views, missing bot tasks on the phone and empty remote Bots groups. Preserve that acceptance gap and check supported native recovery without recreating existing tasks or editing private host state. Empty previews and pending project assignments are diagnostic leads, not a verified repair. Independent project-display-name editing, dedicated icon/color controls and custom-section project ordering also remain unverified. Investigate requested controls through the native surface; use only those actually supported. No filesystem rename substitutes for a missing cosmetic control. Issue #18 owns this bounded guidance/configuration refinement, with independent tests before any new executable behavior and actual repeated-creation/recovery observations for the selected views. Carry per-view results and unresolved limits into aggregate Issue #19. Viewing the same local execution host from another client does not introduce cloud execution.
...
| Owner sidebar addition, Request #9 comment5648891464 | conversational-onboarding; D2/D4; group6 initial/later creation and native grouping verification |
| Owner remote visibility report, Request #9 comment5649621878 | conversational-onboarding selected-client readiness/grouping; D4; group6 per-view acceptance and group7 reconciliation |
| Owner permission discrepancy, Request #9 comment5649648619 | conversational-onboarding observed permission behavior; D4; group6 requested-versus-effective verification |
...
At aggregate closeout, reconcile every accepted requirement and U1–U11 with delivery evidence, synchronize the complete delta into current Specs and archive only after verified integration and independent review. Preserve a compact acceptance/fixity manifest; full execution logs remain on work records. The owner corrected the development continuation mechanism to a Goal. Keep that Goal active through the agreed delivery; complete it only when the actual outcome is achieved. The mistaken recurring continuation hook is paused and is not part of product scheduling.
