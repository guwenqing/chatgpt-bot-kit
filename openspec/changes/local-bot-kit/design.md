...
After matching each managed bot project, use the actual host's native custom-section capabilities to create or reuse the selected group and move only the verified managed projects into it. Resolve prior section identity and actual readback before name matching; missing, renamed or ambiguous sections require reconciliation. Repeated setup and later bot creation reuse the same selected group. Verify placement from the native sidebar inventory; a successful move call alone is not the completed observation. Do not automatically reorganize other projects or infer authority to rename a shared group.

Current actual evidence supports a named section with emoji, moving projects into it and naming conversations. Independent project-display-name editing, dedicated icon/color controls and custom-section project ordering remain unverified. Investigate requested controls through the native surface; use only those actually supported. No filesystem rename or private host-state write substitutes for a missing cosmetic control. Issue #18 owns this bounded guidance/configuration refinement, with independent tests before any new executable behavior and actual repeated-creation/recovery observations.
